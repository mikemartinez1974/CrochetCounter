...
(function (window) {
    ...
    /**
     color validation by Dave Alger.
     http://stackoverflow.com/users/1336975/davealger
     (snagged off stack overflow.)
    **/

    Colors.getColorCSS = function (c) {
        var ele = document.createElement("div");
        ele.style.color = c;
        return ele.style.color.split(/\s+/).join('').toLowerCase();
    };

    Colors.isColorValid = function (c) {
        var s = Colors.getColorCSS(c);
        return (s) ? true : false;
    };

    Colors.isColorTransparent = function (c) {
        var s = Colors.getColorCSS(c);
        return (
        s === "transparent" ||
    ...
    Colors.isColorWhite = function (c) {
        var s = Colors.getColorCSS(c);
        return [
                "white",
                "rgb(255,255,255)",
    ...
    Colors.isColorBlack = function (c) {
        var s = Colors.getColorCSS(c);
        return [
                "black",
                "rgb(0,0,0)",
    ...
    Colors.checkColorString = function () {
        ...
        else if (Colors.isColorValid(c)) {
            if (Colors.isColorTransparent(c)) {
                document.getElementsByTagName("body")[0].style.backgroundColor = 'transparent';
                document.getElementById("result").innerHTML = '<span style="color:#dadada;">TRANSPARENT</span>';
            }
            else if (Colors.isColorWhite(c)) {
                document.getElementsByTagName("body")[0].style.backgroundColor = Colors.getColorCSS(c);
                document.getElementById("result").innerHTML = '<span style="color:black;">WHITE</span>';
            }
            else if (Colors.isColorBlack(c)) {
                document.getElementsByTagName("body")[0].style.backgroundColor = Colors.getColorCSS(c);
                document.getElementById("result").innerHTML = '<span style="color:white;">BLACK</span>';
            }
            else {
                document.getElementsByTagName("body")[0].style.backgroundColor = Colors.getColorCSS(c);
                document.getElementById("result").innerHTML = Colors.getColorCSS(c);
            }
        }
        else {
            document.getElementsByTagName("body")[0].style.backgroundColor = 'transparent';
